...
train = pd.read_csv('netflix.csv', encoding=result['encoding'])
# #print(train.id)
print(train.shape)
print(train.columns.values)

print(train["genre"][0]) #>>>>>>>>>>>>>>>>>>String List (Text)
# Initialize the BeautifulSoup object on a single movie review
# example1 = BeautifulSoup(train["genre"][0] , "html.parser") #>>>>>>>>>>>>>>>>String list (Text)
# # # Print the raw review and then the output of get_text(), for comparison
# print(train["genre"][0]) #>>>>>>>>>>>>>>. Sring
...
#  stemmed_meaningful_words = [stemmer.stem(w) for w in meaningful_words]
 ...
 return( " ".join( words )) 
clean_review = review_to_words( train["genre"][0] )
print(clean_review)
print("----------------------------")
# # Get the number of reviews based on the dataframe column size
num_reviews = train["genre"].size
print(num_reviews)
print("----------------------------")
# Initialize an empty list to hold the clean reviews
clean_train_reviews = []
# #clean_train_reviews.append( review_to_words(train["plot"][0] ) )
# #clean_train_reviews.append( review_to_words(train["plot"][1] ) )
# print("works")
